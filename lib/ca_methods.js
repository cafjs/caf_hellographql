...
exports.methods = {
    ...
    async jitter() {
        if (this.state.weatherInfo.temp) {
            this.state.weatherInfo.temp = this.state.weatherInfo.temp + 1;
            // Don't change humidity !
            // this.state.weatherInfo.humidity =
            //    this.state.weatherInfo.humidity + 1;
            this.state.weatherInfo.wind = this.state.weatherInfo.wind + 1;
            // ugly, simulate a weather query response
            this.__ca_handleWeather__(myUtils.uniqueId(),
                                      [null, this.state.weatherInfo]);
        }
        return this.getState();
    },
